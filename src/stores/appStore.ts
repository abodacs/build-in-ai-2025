    ...
    (set, get) => ({
      ...
      setApiResult: (result: ApiResult) => {
        try {
          ...
          set({ apiResult: validated, isLoading: false }, false, 'setApiResult')
        } catch (error) {
...
export const useAiCapabilities = () => useAppStore(state => state.aiCapabilities)
export const useLastInputText = () => useAppStore(state => state.lastInputText)
