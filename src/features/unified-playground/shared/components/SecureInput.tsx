...
export const SecureInput = forwardRef<HTMLTextAreaElement, SecureInputProps>(({
  ...
  const validateInput = useCallback(async (inputValue: string) => {
    ...
    // Rate limiting check
    if (enableRateLimit && !rateLimiter.isAllowed(`input-${apiType}`, 30, 60000)) {
      setValidation({
        isValid: false,
    ...
    if (result.errors.length > 0) {
      securityLevel = result.errors.some(e =>
        e.includes('unsafe') || e.includes('suspicious')
      ) ? 'danger' : 'warning'
    }
...
export const SecureQuickInput = forwardRef<HTMLInputElement, Omit<SecureInputProps, 'variant'> & {
  type?: 'text' | 'email' | 'url'
}>(({
  value,
  onChange,
