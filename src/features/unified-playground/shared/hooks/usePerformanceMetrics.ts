...
export function usePerformanceMetrics() {
  ...
  const measureApiPerformance = useCallback(async <T>(
    apiCall: () => Promise<T>,
    apiType: string
  ): Promise<T> => {
    ...
    try {
      ...
      setHistory(prev => [...prev.slice(-50), {
        timestamp: Date.now(),
        metrics: { apiResponseTime: responseTime, memoryUsage: memoryDelta },
        apiType,
        action: 'api_call'
      }])
    ...
    try {
      if ('navigator' in window && 'connection' in navigator) {
        ...
        if (connection && connection.effectiveType) {
          ...
          const estimatedSize = (loadTime / 1000) * bandwidth * 1024 * 1024 / 8 // bytes
  ...
  const startMonitoring = useCallback(() => {
    ...
    detectBundleSize()
  }, [initializeCoreWebVitals, getNavigationMetrics, startFrameRateMonitoring, detectBundleSize])
  ...
  const getOptimizationSuggestions = useCallback((): string[] => {
    ...
    if (metrics.lcp && metrics.lcp > 2500) {
      suggestions.push('Optimize Largest Contentful Paint: Consider lazy loading or image optimization')
    }
    ...
    if (metrics.cls && metrics.cls > 0.1) {
      suggestions.push('Improve Cumulative Layout Shift: Set dimensions for images and ads')
    }

    if (metrics.apiResponseTime && metrics.apiResponseTime > 1000) {
      suggestions.push('Optimize API calls: Consider caching or request debouncing')
    }

    if (metrics.memoryUsage && metrics.memoryUsage > 50 * 1024 * 1024) {
      suggestions.push('Reduce memory usage: Check for memory leaks or large object retention')
    }

    if (metrics.frameRate && metrics.frameRate < 50) {
      suggestions.push('Improve frame rate: Optimize animations and reduce DOM manipulation')
    }
...
export function usePerformanceContext() {
  ...
  const withPerformanceTracking = useCallback(<T extends any[], R>(
    ...
    return (...args: T): R => {
      const stopMeasuring = performanceMetrics.measureInteractionLatency(label)
