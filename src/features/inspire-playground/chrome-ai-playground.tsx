...
export function ChromeAIPlayground() {
  const [selectedAPI, setSelectedAPI] = useState<keyof APIConfigs>("summarizer")
  const [activeTab, setActiveTab] = useState("demo")
  const [inputText, setInputText] = useState("")
  const [output, setOutput] = useState("")
  const [isProcessing, setIsProcessing] = useState(false)
  const [performanceMetrics, setPerformanceMetrics] = useState<{ duration: number; tokens?: number } | null>(null)
  ...
  const currentAPI = useMemo(() => apis.find((api) => api.id === selectedAPI), [apis, selectedAPI])
  ...
  const contextAnalysis = useMemo(() => {
    ...
    const efficiency = Math.min(100, (charCount / limits.recommendedChars) * 100)
  ...
  const updateAPIConfig = useCallback(<T extends keyof APIConfigs>(apiId: T, updates: Partial<APIConfigs[T]>) => {
    setApiConfigs((prev) => ({
      ...prev,
        ...
        if (file.type.startsWith("image/")) {
          ...
          reader.onload = (e) => {
            setMultimediaFiles((prev) =>
              prev.map((f) => (f.id === multimediaFile.id ? { ...f, preview: e.target?.result as string } : f)),
            )
          }
  ...
  const chunkingSuggestions = useMemo(() => {
    ...
    return chunks
  }, [inputText, contextAnalysis?.canChunk, currentAPI?.contextLimits?.recommendedChars])
  ...
  useEffect(() => {
    ...
    const checkAPIAvailability = async () => {
      ...
      console.log("[v0] Checking Chrome AI API availability...")
    }
  ...
  const handleAPICall = useCallback(async () => {
    ...
    try {
      ...
      startTransition(() => {
        ...
        setPerformanceMetrics({ duration, tokens: Math.floor(inputText.length / 4) })
        setHasRunDemo(true)
      })
    } catch (error) {
      startTransition(() => {
  ...
  topK: ${promptConfig.topK}${promptConfig.systemPrompt ? `,\n  systemPrompt: '${promptConfig.systemPrompt}'` : ""}
    ...
    const inputParam =
      selectedAPI === "prompt" && hasMultimedia ? "multimediaInput" : `'${sampleInput.replace(/'/g, "\\'")}'`
...
${selectedAPI}Example()
  ...
  .catch(error => console.error('Error:', error));`
    ...
    return apiCall.trim()
  }, [codeLanguage, inputText, multimediaFiles, selectedAPI, currentConfig, currentAPI?.name])
    ...
    switch (selectedAPI) {
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value: SummarizerConfig["type"]) => updateAPIConfig("summarizer", { type: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value: SummarizerConfig["length"]) => updateAPIConfig("summarizer", { length: value })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              <Label htmlFor="summarizer-context">Shared Context (Optional)</Label>
              <Input
                ...
                onChange={(e) => updateAPIConfig("summarizer", { sharedContext: e.target.value })}
              />
            </div>
          </div>
        )
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value: WriterConfig["tone"]) => updateAPIConfig("writer", { tone: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value: WriterConfig["format"]) => updateAPIConfig("writer", { format: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value: WriterConfig["length"]) => updateAPIConfig("writer", { length: value })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateAPIConfig("writer", { sharedContext: e.target.value })}
              />
            </div>
          </div>
        )
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value: RewriterConfig["tone"]) => updateAPIConfig("rewriter", { tone: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value: RewriterConfig["format"]) => updateAPIConfig("rewriter", { format: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value: RewriterConfig["length"]) => updateAPIConfig("rewriter", { length: value })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              <Label htmlFor="rewriter-context">Shared Context (Optional)</Label>
              <Input
                id="rewriter-context"
                placeholder="e.g., Professional email communication"
                value={rewriterConfig.sharedContext}
                onChange={(e) => updateAPIConfig("rewriter", { sharedContext: e.target.value })}
              />
            </div>
          </div>
        )
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value: string) => updateAPIConfig("translator", { sourceLanguage: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value: string) => updateAPIConfig("translator", { targetLanguage: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
      ...
      case "proofreader":
        ...
        return (
          <div className="space-y-4">
            <div>
              <Label htmlFor="proofreader-languages">Expected Input Languages</Label>
              <Select
                value={proofreaderConfig.expectedInputLanguages[0] || "en"}
                onValueChange={(value: string) => updateAPIConfig("proofreader", { expectedInputLanguages: [value] })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
      ...
      case "prompt":
        ...
        return (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="prompt-temperature">Temperature: {promptConfig.temperature}</Label>
                <Slider
                  ...
                  onValueChange={([value]) => updateAPIConfig("prompt", { temperature: value })}
                  className="mt-2"
                />
                <p className="text-xs text-muted-foreground mt-1">Controls creativity (0 = focused, 1 = creative)</p>
              </div>
              <div>
                ...
                <Slider
                  ...
                  onValueChange={([value]) => updateAPIConfig("prompt", { topK: value })}
                  className="mt-2"
                />
                <p className="text-xs text-muted-foreground mt-1">Vocabulary diversity (1 = focused, 40 = diverse)</p>
              </div>
            </div>
            <div>
              ...
              <Textarea
                ...
                onChange={(e) => updateAPIConfig("prompt", { systemPrompt: e.target.value })}
                className="mt-2"
              />
            </div>
          </div>
        ...
        return (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Language Detection API has minimal configuration options. It automatically detects the language of input
              text.
            </p>
          </div>
        )
    ...
    <div className="min-h-screen grid-pattern">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Interactive playground for Chrome's built-in AI APIs</p>
              </div>
            </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Alert className="mb-8 security-warning">
          ...
          <AlertDescription>
            Chrome AI APIs are currently in development. This playground demonstrates the upcoming functionality. You'll
            need Chrome Canary with experimental flags enabled to test the actual APIs.
          </AlertDescription>
        </Alert>
          ...
          <div className="lg:col-span-1">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Select an API to explore its capabilities</CardDescription>
              </CardHeader>
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                  ...
                  <TabsContent value="demo" className="space-y-6">
                    <div className="bg-muted/50 p-4 rounded-lg border-l-4 border-primary">
                      <p className="text-sm text-muted-foreground">
                        <strong>Step 1:</strong> Configure the API settings below, then try it with your input. Check
                        the <strong>Code</strong> tab to see the implementation with your exact configuration.
                      </p>
                    </div>
                      ...
                      <CollapsibleContent className="space-y-4 mt-4">
                        <Card>
                          <CardHeader className="pb-3">
                            <CardTitle className="text-base">Settings for {currentAPI?.name}</CardTitle>
                            <CardDescription>
                              Configure the API options. Changes will be reflected in the generated code.
                            </CardDescription>
                          </CardHeader>
                          <CardContent>{renderConfigurationPanel()}</CardContent>
                        </Card>
                      </CollapsibleContent>
                    </Collapsible>
                    ...
                    <div className="space-y-4">
                          ...
                          <div>
                            ...
                            >
                              <div className="flex flex-col items-center gap-2">
                                ...
                                <p className="text-xs text-muted-foreground">Supports: JPG, PNG, GIF, MP3, WAV, M4A</p>
                              </div>
                              <input
                                ref={fileInputRef}
                                type="file"
                                ...
                                {multimediaFiles.map((file) => (
                                  <div key={file.id} className="relative group">
                                    ...
                                    <Button
                                      ...
                                      onClick={() => removeMultimediaFile(file.id)}
                                    >
                                      <X className="h-3 w-3" />
                                    </Button>
                                  </div>
                          ...
                          {contextAnalysis && (
                            <div className="flex items-center gap-2">
                              <Badge
                                ...
                                className="text-xs"
                              >
                                {contextAnalysis.charCount.toLocaleString()} chars
                              </Badge>
                              {contextAnalysis.canChunk && (
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => setShowContextTips(!showContextTips)}
                          ...
                          <div className="mt-2 space-y-2">
                            {contextAnalysis.efficiency > 0 && (
                              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                ...
                                <span>Context efficiency: {Math.round(contextAnalysis.efficiency)}%</span>
                                {contextAnalysis.estimatedProcessingTime > 1 && (
                                  <span>• Est. processing: {contextAnalysis.estimatedProcessingTime}s</span>
                                )}
                              </div>
                            )}
                            ...
                            {contextAnalysis.isOverMax && (
                              <Alert className="py-2">
                                ...
                                <AlertDescription className="text-xs">
                                  Input exceeds maximum length ({currentAPI?.contextLimits?.maxChars.toLocaleString()}{" "}
                                  chars). Consider chunking your content.
                                </AlertDescription>
                              </Alert>
                            )}

                            {contextAnalysis.isOverRecommended && !contextAnalysis.isOverMax && (
                              <Alert className="py-2">
                                <Info className="h-4 w-4" />
                                <AlertDescription className="text-xs">
                                  Large input detected. Performance may be slower. Recommended:{" "}
                                  {currentAPI?.contextLimits?.recommendedChars.toLocaleString()} chars or less.
                                </AlertDescription>
                              </Alert>
                            )}
                          </div>
                          ...
                          <Card className="mt-3">
                            <CardHeader className="pb-2">
                              ...
                              <CardDescription className="text-xs">
                                Your input is large. Consider processing it in chunks for better performance.
                              </CardDescription>
                            </CardHeader>
                            <CardContent className="pt-2">
                              <div className="space-y-2">
                                <p className="text-xs text-muted-foreground">
                                  Suggested chunks: {chunkingSuggestions.length} parts
                                </p>
                                <div className="grid grid-cols-2 gap-2 text-xs">
                                  {chunkingSuggestions.slice(0, 4).map((chunk) => (
                                    <div key={chunk.index} className="bg-muted/50 p-2 rounded">
                                ...
                                {chunkingSuggestions.length > 4 && (
                                  <p className="text-xs text-muted-foreground">
                                    +{chunkingSuggestions.length - 4} more chunks...
                                  </p>
                                )}
                              </div>
                            </CardContent>
                      ...
                      <div className="flex items-center gap-4">
                        <Button
                          ...
                          disabled={
                            (!inputText.trim() && multimediaFiles.length === 0) ||
                            isProcessing ||
                  ...
                  <TabsContent value="code" className="space-y-6">
                    {!hasRunDemo && (
                      <div className="bg-muted/50 p-4 rounded-lg border-l-4 border-muted-foreground">
                        <p className="text-sm text-muted-foreground">
                          <strong>Tip:</strong> Configure the API and run the demo first to see how the code works with
                          your specific configuration and input.
                        </p>
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <Select
                          value={codeLanguage}
                          onValueChange={(value: "javascript" | "typescript") => setCodeLanguage(value)}
                        >
                    ...
                    {hasRunDemo && (
                      <div className="bg-accent/10 p-4 rounded-lg">
                        <p className="text-sm text-muted-foreground">
                          ...
                          {multimediaFiles.length > 0 ? ", multimedia files," : ""} and demo input, and will produce
                          similar results to what you just tested.
                        </p>
                      </div>
                    )}
                  </TabsContent>
                    ...
                    {selectedAPI === "prompt" ? (
                      <div className="space-y-4">
                        <Alert className="security-warning">
                          ...
                          <AlertDescription>
                            <strong>Security Warning:</strong> The Prompt API is vulnerable to prompt injection attacks.
                            Always validate and sanitize user inputs.
                          </AlertDescription>
                        </Alert>

                        <div className="space-y-4">
                          <h3 className="text-lg font-semibold">Prompt Injection Prevention</h3>
                          <p className="text-muted-foreground">
                            Learn how to protect your application from malicious prompts that could manipulate AI
                            behavior.
                          </p>
...
\`;`}
                      ...
                      <div className="space-y-4">
                        ...
                        <p className="text-muted-foreground">
                          Security best practices for {currentAPI?.name} will be documented here.
                        </p>
                        ...
                        <Card>
                          ...
                          <CardContent>
                            <p className="text-sm text-muted-foreground">
                              All processing happens on-device. Your data never leaves your browser.
                            </p>
                          </CardContent>
                        </Card>
                      </div>
